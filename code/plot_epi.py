...
def plot_epipolar_lines(image1, image2, uncalibrated_1, uncalibrated_2, E, K, plot=True):
    ...
    K_inv_trans = np.linalg.inv(K).T
    K_inv = np.linalg.inv(K)
    F_Matrix = np.matmul(K_inv_trans,np.matmul(E,K_inv))
    #calculating the epipolar lines
    epipolar_lines_in_1 = np.matmul(F_Matrix.T,uncalibrated_2)
    epipolar_lines_in_2 = np.matmul(F_Matrix,uncalibrated_1)
    #epipolar_lines_in_2 = np.matmul(F_Matrix.T,uncalibrated_1.T)
